...
# 이미지 사이즈
    ...
    im_size=uploadedfile.size  #integer
...
w_name = None
with col1:
    w_hakbun = st.empty()  # empty 컨테이너(w_hakbun): 동시에 하나의 위젯만 담을 수 있는 그릇
...
if picture:  # 캡쳐한 이미지 파일
    ...
    hakbun = w_hakbun.text_input('학번', max_chars=4, key='w_hakbun_2') # 키를 지정해서 다른 위젯으로 인식
